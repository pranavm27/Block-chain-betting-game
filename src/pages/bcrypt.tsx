...
const component = () => {
  ...
  const converFromBase64 = () => {
    var plainText = CryptoJS.enc.Utf8.stringify(
      CryptoJS.enc.Base64.parse(encodedValue)
    );
  ...
  const [value, setValue] = useState("plain text");
  const [encodedValue, setEncodedValue] = useState("cGxhaW4gdGV4dA==");


  const[plainText , setPlainText] =useState('test@123')
  const[encyptedValue, setEncyptedValue] = useState('test');
